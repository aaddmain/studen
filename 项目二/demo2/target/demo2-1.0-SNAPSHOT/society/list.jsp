    ...
    <div class="main-container">
        ...
        <main class="main-content">
            ...
            <div class="content">
                <!-- 页面标题 -->
                <h1 class="page-title">
                    <i class="fa fa-list"></i> 协会列表
                </h1>
                
                <!-- 工具栏 -->
                <div class="toolbar">
                    <form action="/demo2_war_exploded/society" method="get" class="search-box">
                        <input type="hidden" name="action" value="list">
                        <input type="text" placeholder="搜索协会名称..." name="keyword">
                        <button type="submit" class="btn btn-primary">
                            <i class="fa fa-search"></i> 搜索
                        </button>
                    </form>
                    <a href="/demo2_war_exploded/society?action=create" class="btn btn-primary">
                        ...
                        <tbody>
                            <c:forEach items="${societies}" var="society">
                                <tr>
                                    ...
                                    <td>${society.societyCreateDate}</td>
                                    <td>${society.societyCreator}</td>
                                    <td>${society.societyIntro}</td>
                                    <td>
                                        <c:choose>
                                            <c:when test="${society.societyStatus == 1}">
                                                <span class="status-tag status-normal">正常</span>
                                            </c:when>
                                            <c:when test="${society.societyStatus == 2}">
                                                <span class="status-tag status-paused">停招</span>
                                            </c:when>
                                            <c:otherwise>
                                                <span class="status-tag status-stopped">停运</span>
                                            </c:otherwise>
                                        </c:choose>
                                    </td>
                                    <td>
                                        <div class="action-buttons">
                                            <a href="/demo2_war_exploded/society?action=detail&id=${society.societyId}" class="btn btn-secondary">
                                                <i class="fa fa-eye"></i> 详情
                                            </a>
                                            <a href="/demo2_war_exploded/society?action=edit&id=${society.societyId}" class="btn btn-primary">
                                                <i class="fa fa-pencil"></i> 编辑
                                            </a>
                                            <form action="/demo2_war_exploded/society" method="post" style="display: inline;">
                                                <input type="hidden" name="action" value="delete">
                                                <input type="hidden" name="id" value="${society.societyId}">
                                                <button type="submit" class="btn btn-danger" onclick="return confirm('确定要删除这个协会吗？')">
